...
int stock[5][3]={{0,0,0},{1,12,100},{2,15,200},{3,17,150},{4,25,250}};// stock of goods
typedef struct{
     int code;
int quantity;
    float cost;
}cartitems; // to create structure of code quantity cost of a good
cartitems c[10];    //array of structure
int inum=0;
float total=0; // initializing totalbill =0
...
scanf("%d",&choice);
    ...
    switch(choice)
    {
        case 1:additem();//adds the item to cart
            break;
        case 2:deleteitem();//deletes the last item which was added to cart
            break;
        case 3:displaycart();// displays the cart with total bill
            break;
        default: exit(0);// to stop shopping
    };
}while(1);
    return 0;
}
